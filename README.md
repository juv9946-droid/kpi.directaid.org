...
نظام ويب مؤسسي لإدارة ومتابعة مؤشرات الأداء — واجهة عربية (RTL) تعمل على الكمبيوتر والجوال بالمتصفح فقط، مع خادم Node.js + Express وقاعدة بيانات MySQL مركزية. رابط واحد وحساب مشترك؛ وأي تعديل يقوم به أي موظف يظهر عند البقية **في نفس اللحظة** عبر بث لحظي دائم (Server-Sent Events).
...
│   └── cloud-sync.js     # طبقة المزامنة اللحظية مع القاعدة المركزية
├── assets/               # ملفات عامة
├── images/               # الصور
├── fonts/                # الخطوط (تُحمّل حاليًا من Google Fonts)
└── server/               # الخادم (Backend)
    ├── app.js            # نقطة التشغيل
    ├── realtime.js       # محرّك البث اللحظي (SSE) لكل المتصفحات المفتوحة
    ├── package.json
    ├── .env.example
    ├── routes/kpi.js
...
| GET    | `/api/stream`  | بث لحظي دائم للتعديلات (SSE)    |
...
## أين تُحفظ البيانات؟
كل البيانات تُحفظ في **قاعدة MySQL السحابية فقط** (جدول `kpi_store`). المتصفح لا يُستخدم كمخزن دائم — يوجد فقط ذاكرة عرض مؤقتة تُعاد تعبئتها من القاعدة عند كل فتح، فلا شيء يضيع عند إعادة تشغيل الموقع أو مسح المتصفح.
...
- **المزامنة اللحظية:** كل متصفح مفتوح يحتفظ باتصال بث دائم مع الخادم (`/api/stream`). عند أي تعديل يُحفظ في MySQL ثم يُدفع فورًا لبقية المتصفحات وتُحدَّث الشاشة تلقائيًا — بلا زر تحديث وبلا انتظار.
- إن انقطع الاتصال (شبكة ضعيفة أو وسيط يقطع البث) يعود النظام تلقائيًا للسحب الدوري كل ٢.٥ ثانية، ثم يستأنف البث اللحظي عند توفّره.
- إذا كان المستخدم يكتب في حقل، تُطبَّق تعديلات الآخرين بعد انتهائه من الكتابة حتى لا يفقد ما يكتبه.
- زر التحديث الأزرق أسفل الشاشة يسحب أحدث نسخة من القاعدة يدويًا عند الحاجة، وجرس الإشعارات يعرض سجل آخر التحديثات.
